...
SELECT tablename, attname, n_distinct, -- Estimated unique values
...
```sql
SELECT datname, numbackends, -- Active connections
...
```sql
SELECT schemaname, relname, seq_scan, -- Sequential scans (high = may need index)
  seq_tup_read, idx_scan, -- Index scans
  idx_tup_fetch, n_tup_ins, n_tup_upd, n_tup_del, n_tup_hot_upd, -- HOT updates (good, in-page updates)
...
-- Index hit ratio
SELECT schemaname, relname, indexrelname, idx_scan, idx_tup_read, idx_tup_fetch,
  CASE WHEN idx_tup_read > 0 THEN round(100.0 * idx_tup_fetch / idx_tup_read, 2) ELSE 0 END as hit_ratio
FROM pg_stat_user_indexes
WHERE idx_scan > 0
ORDER BY hit_ratio;
```
...
```sql
-- Current locks
SELECT l.pid, a.usename, a.query, l.mode, l.locktype, l.granted, l.relation::regclass
FROM pg_locks l
