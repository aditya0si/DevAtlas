...
```sql
-- -> returns JSONB
SELECT data -> 'user' FROM documents;
-- {"id": 123, "name": "Alice"}
...
-- ->> returns text
SELECT data ->> 'status' FROM documents;
-- active (text)
...
SELECT data #> '{user,address,city}' FROM documents;
-- "NYC" (JSONB)
...
SELECT * FROM documents WHERE data ->> 'title' ILIKE '%postgres%';
```
...
FROM documents WHERE data @> '{"status": "active"}';

-- Object aggregation
SELECT jsonb_object_agg(id, data -> 'user') as user_map
FROM documents WHERE data ? 'user';
```
...
SELECT id, jsonb_array_elements(data -> 'tags') as tag FROM documents;

-- Expand array to text
SELECT id, jsonb_array_elements_text(data -> 'tags') as tag FROM documents;

-- Array length
SELECT * FROM documents WHERE jsonb_array_length(data -> 'tags') > 5;

-- Filter array elements
SELECT id, jsonb_path_query_array(data, '$.tags[*] ? (@ like_regex "^post.*" flag "i")') as postgres_tags
FROM documents;
```
...
-- Exists check
SELECT * FROM documents WHERE jsonb_path_exists(data, '$.tags[*] ? (@ == "postgresql")');
