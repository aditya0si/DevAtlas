...
from app.models.github import AnalyticsSnapshot, Repository
    ...
    async with async_session_factory() as session:
        ...
        try:
            ...
            top_lang_query = await session.execute(text(
                "SELECT language, COUNT(*) FROM repositories WHERE language IS NOT NULL "
            ...
            top_city_query = await session.execute(text(
                "SELECT city, COUNT(*) FROM github_users WHERE country = 'India' "
                "AND city IS NOT NULL GROUP BY city ORDER BY count DESC LIMIT 5"
            ))
